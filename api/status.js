...
const FORCE_REFRESH_MIN_MS = 3 * 60 * 1000;
const MAX_ITEMS = 50;
  ...
  try {
    const results = await Promise.all(
      feeds.map(async (feed) => {
        ...
        if (!xmlText) return [];
        return parseFeed(xmlText, feed.service);
      })
    );
    ...
    return res.status(500).json({ error: 'Status API failed', details: error.message });
  }
}
...
function parseFeed(xmlText, service) {
  const items = [];
  ...
  blocks.forEach((block) => {
    const title = decodeEntities(getTagValue(block, 'title')) || 'Status update';
    ...
    items.push({
      id: createId(`${service}-${link || title}-${publishedISO}`),
      service,
      title: title.trim(),
      statusType: deriveStatusType(`${title} ${summary}`),
      publishedISO,
      url: link,
      summary
    });
...
function deriveStatusType(text) {
  const normalized = text.toLowerCase();
  if (normalized.includes('resolved')) return 'resolved';
  if (normalized.includes('maintenance')) return 'maintenance';
  if (normalized.includes('investigating') || normalized.includes('identified') || normalized.includes('monitoring') || normalized.includes('incident') || normalized.includes('outage') || normalized.includes('degraded')) {
    return 'incident';
  }
  return 'incident';
}
